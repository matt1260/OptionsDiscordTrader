...
def is_market_open():
    ...
    for session in market_hours:
        start_time = datetime.datetime.fromisoformat(
            session['start']).astimezone(ny_timezone)
        end_time = datetime.datetime.fromisoformat(
            session['end']).astimezone(ny_timezone)
    ...
    if is_market_open():
        ...
        if spy_pivot_alert is not None:
            ...
            if spy_pivot_alert in [last_alert, last_alert2, last_alert3]:
                ...
                symbol = 'SPY'
                ITM_contract, option_entry_price = build_0dte_order(
                    'long', symbol)
                ...
                retStr = str("""```diff\n+ """ + spy_pivot_alert + ' Entry Price: ' + str(
                    entry_price) + ', ' + ITM_contract + ' @ ' + str(option_entry_price) + """```""")
                print(retStr)

                chart_buffer = create_pivot_chart(df)

                record = [time, symbol, low, spy_pivot_alert, spy_type,
                ...
                if pnl < 0:
                    close_alert = symbol + " Bull Pivot loss (10m) since last alert at " + entry_time + ": $" + str(
                        pnl) + ' - ' + ITM_contract + ': $' + str(option_pnl)
                    color = 'red'
                    ...
                    close_alert = symbol + " Bull Pivot gain (10m) since last alert at " + entry_time + ": $" + str(
                        pnl) + ' - ' + ITM_contract + ': $' + str(option_pnl)
                    color = 'green'
                ...
                embed.add_field(
                    name='10 minute P/L for last SPY alert', value=retStr, inline=False)
                if color == 'green':
            ...
            if qqq_pivot_alert in [last_alert, last_alert2, last_alert3]:
                ...
                retStr = str("""```diff\n+ """ + qqq_pivot_alert + ' Entry Price: ' + str(
                    entry_price) + ', ' + ITM_contract + ' @ ' + str(option_entry_price) + """```""")
                ...
                record = [time, symbol, low, qqq_pivot_alert, qqq_type,
                          entry_price, ITM_contract, option_entry_price]
        ...
        if qqq_pivot_alert is not None:
            ...
            if qqq_pivot_alert in [last_alert, last_alert2, last_alert3]:
                ...
                retStr = str(qqq_pivot_alert + ' Entry Price: ' + str(entry_price) +
                             ', ' + ITM_contract + ' @ ' + str(option_entry_price))
                print(retStr)
        ...
        if spx_pivot_alert is not None:
            ...
            if spx_pivot_alert in [last_alert, last_alert2, last_alert3]:
                ...
                retStr = str("""```diff\n+ """ + spx_pivot_alert + ' Entry Price: ' + str(
                    entry_price) + ', ' + ITM_contract + ' @ ' + str(option_entry_price) + """```""")
                print(retStr)
                ...
                record = [time, symbol, low, spx_pivot_alert, spx_type,
                          entry_price, ITM_contract, option_entry_price]
                ...
                embed = discord.Embed(title="\U0001F3E6 PnL")
                embed.add_field(
                    name='10 minute P/L for last SPX alert', value=retStr, inline=False)
                if color == 'green':
                    embed.color = 0x00FF00
                elif color == 'red':
                    embed.color = 0xFF0000
...
async def alert(ctx, arg):
    ...
    if arg == 'spxcall':
        ...
        record = [entry_time, symbol, pivot_low, spx_pivot_alert,
                  spx_type, entry_price, ITM_contract, option_entry_price]
        ...
        if pnl < 0:
            ...
            print("Setting stoploss at breakeven...")
            ...
            while True:
                ...
                if last == (entry_price + 1.00):
                    ...
                    close_alert = "Stopped out. " + symbol + \
                        " Bull Pivot gain (10m) since last alert at " + entry_time + ": $" + str(
                            pnl) + ', ' + ITM_contract + ': $' + str(option_pnl)
                    color = 'green'
                ...
                # take profit at 20%
                if option_last > (option_entry_price + (option_entry_price * .2)):
                    close_alert = "20% gain on call. " + symbol + " Bull Pivot gain since last alert at " + \
                        entry_time + ": $" + \
                        str(pnl) + ', ' + ITM_contract + \
                        ': $' + str(option_pnl)
                    color = 'green'
    ...
    if arg == 'spxput':
        ...
        retStr = str("""```""" + spx_pivot_alert + ' Entry Price: ' + str(entry_price) +
                     ', ' + ITM_contract + ' @ ' + str(option_entry_price) + """```""")
        print(retStr)
        ...
        record = [entry_time, symbol, pivot_high, spx_pivot_alert,
                  spx_type, entry_price, ITM_contract, option_entry_price]
        ...
        if pnl < 0:
            close_alert = symbol + " Bear Pivot loss (10m) since last alert at " + entry_time + ": $" + str(
                pnl) + ', ' + ITM_contract + ': $' + str(option_pnl)
            color = 'red'
            ...
            close_alert = symbol + " Bear Pivot gain (10m) since last alert at " + entry_time + ": $" + str(
                pnl) + ', ' + ITM_contract + ': $' + str(option_pnl)
            color = 'green'
